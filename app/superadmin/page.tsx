...
interface Tenant {
  ...
  settings?: { app_name: string; primary_color: string; operator_name?: string; logo_url?: string | null; skin?: string | null };
...
export default function SuperAdminPage() {
  ...
  const S: Record<string, React.CSSProperties> = {
    input: { width: "100%", background: "var(--sk-deep)", border: "0.5px solid var(--sk-surface)", borderRadius: 8, padding: "10px 14px", color: "var(--sk-t1)", boxSizing: "border-box" as const },
    smallInput: { width: "100%", background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 6, padding: "8px 10px", color: "var(--sk-t1)", fontSize: 12, boxSizing: "border-box" as const },
    label: { fontSize: 11, color: "#6b7280", display: "block" as const, marginBottom: 4 },
  };

  if (!authed) return (
    <div style={{ minHeight: "100vh", background: "var(--sk-deep)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <form onSubmit={(e) => { e.preventDefault(); verifyAndLogin(); }}
        style={{ background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 16, padding: 40, width: 340, maxWidth: "92vw" }}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--sk-t2)", marginBottom: 14, cursor: "pointer" }}>
          <input type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)} />
          Rester connecté sur cet appareil
        </label>
        <button type="submit"
          style={{ width: "100%", background: "#f5a623", color: "var(--sk-deep)", border: "none", borderRadius: 8, padding: 10, fontWeight: 700, cursor: "pointer" }}>
          Accéder →
        </button>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--sk-deep)", padding: "32px 24px", color: "var(--sk-t1)", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 980, margin: "0 auto" }}>

        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
          <div style={{ background: "#f5a623", borderRadius: 8, width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "var(--sk-deep)" }}>M3</div>
          <div>
        ...
        {activeTab === "settings" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
            ...
            <div style={{ background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 12, padding: 24, gridColumn: "1/-1" }}>
            ...
            <div style={{ background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 12, padding: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <button onClick={changeKey}
                  style={{ background: "#f5a623", color: "var(--sk-deep)", border: "none", borderRadius: 8, padding: "11px", fontWeight: 700, cursor: "pointer", marginTop: 4 }}>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={() => saveGlobalSetting("whatsapp", globalSettings.whatsapp)} disabled={settingsLoading}
                      style={{ background: "#22c55e", color: "var(--sk-deep)", border: "none", borderRadius: 8, padding: "10px 14px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                      ✓
                    </button>
                  </div>
                </div>
            ...
            <div style={{ background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 12, padding: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ background: "var(--sk-deep)", borderRadius: 8, padding: 12, marginTop: 4 }}>
            ...
            <div style={{ background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 12, padding: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  ...
                  <div key={k} style={{ display:"flex", justifyContent:"space-between", borderBottom:"0.5px solid var(--sk-surface)", paddingBottom:6 }}>
                    <span style={{ fontSize:11, color:"#6b7280" }}>{k}</span>
                    <span style={{ fontSize:11, color:"var(--sk-t1)" }}>{v}</span>
                  </div>
                ))}
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 32, flexWrap: "wrap" }}>
          {(Object.entries(PLAN_LIMITS) as [Plan, typeof PLAN_LIMITS[Plan]][]).map(([plan, l]) => (
            <div key={plan} style={{ background: "var(--sk-bg)", border: `0.5px solid ${PLAN_COLORS[plan]}40`, borderRadius: 10, padding: "10px 16px", flex: 1, minWidth: 180 }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          {loading ? <p style={{ color: "#6b7280" }}>Chargement...</p> : tenants.map(t => {
            ...
            return (
              <div key={t.id} style={{ background: "var(--sk-bg)", border: `0.5px solid ${trialStatus.state === "expired" ? "#ef444430" : "var(--sk-surface)"}`, borderRadius: 12, marginBottom: 8 }}>
                <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <div style={{ width: 30, height: 30, borderRadius: 6, background: t.settings?.primary_color || "#f5a623", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "var(--sk-deep)", flexShrink: 0 }}>
                  ...
                  <div style={{ borderTop: "0.5px solid var(--sk-surface)", padding: 20 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
                      ...
                      <div>
                        ...
                        {(t.admins || []).map(a => (
                          <div key={a.id} style={{ background: "var(--sk-deep)", borderRadius: 8, marginBottom: 5 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px" }}>
                              <div style={{ width: 24, height: 24, borderRadius: 5, background: "var(--sk-surface)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: "#9ca3af", flexShrink: 0 }}>
                                {a.email.slice(0, 1).toUpperCase()}
                              </div>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: 11, color: "var(--sk-t1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.email}</div>
                              </div>
                            ...
                            {editAdmin[a.id] && (
                              <div style={{ padding: "0 10px 10px" }}>
                                ...
                                <button onClick={() => updateAdmin(a.id)}
                                  style={{ background: "#f5a623", color: "var(--sk-deep)", border: "none", borderRadius: 6, padding: "5px 12px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>
                                  Sauvegarder
                                </button>
                              </div>
                            )}
                        ...
                        <div style={{ background: "var(--sk-deep)", borderRadius: 8, padding: 10, marginTop: 8 }}>
                          ...
                          <button onClick={() => createAdmin(t.id)}
                            style={{ background: "#f5a623", color: "var(--sk-deep)", border: "none", borderRadius: 6, padding: "7px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>
                            + Créer admin
                          </button>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{ background: "var(--sk-deep)", borderRadius: 8, padding: 12, marginBottom: 8 }}>
                          {[
                            ...
                            <div key={l} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 5 }}>
                              ...
                              <span style={{ color: "var(--sk-t1)" }}>{v}</span>
                            </div>
                          ))}
                        </div>
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          <button onClick={() => extendAccess(t.id)}
                            style={{ background: "#22c55e", color: "var(--sk-deep)", border: "none", borderRadius: 6, padding: "7px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>
                            Étendre
                          </button>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{ background: "var(--sk-deep)", borderRadius: 8, padding: 12 }}>
                          {["14d", "7d", "3d", "1d", "expired"].map(h => {
                            ...
                            return (
                              <div key={h} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                                <span style={{ width: 8, height: 8, borderRadius: "50%", background: sentAt ? HORIZON_COLORS[h] : "var(--sk-surface)", display: "inline-block", flexShrink: 0 }} />
                                <span style={{ fontSize: 11, color: sentAt ? "var(--sk-t1)" : "#374151", flex: 1 }}>
                                  {h === "expired" ? "Expiration" : `J-${h.replace("d", "")}`}
                                </span>
                                <span style={{ fontSize: 10, color: sentAt ? "#6b7280" : "var(--sk-surface)" }}>
        ...
        <div style={{ background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 12, padding: 24 }}>
          ...
          <button onClick={createTenant}
            style={{ background: "#f5a623", color: "var(--sk-deep)", border: "none", borderRadius: 8, padding: "12px 24px", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>
            + Créer le client
          </button>
        </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
        {(["admin_confirmed", "injected", "rejected"] as const).map((s) => (
          <button key={s} onClick={() => setStatusFilter(s)}
            style={{ background: statusFilter === s ? "#f5a623" : "var(--sk-surface)", color: statusFilter === s ? "var(--sk-deep)" : "#9ca3af", border: "none", borderRadius: 8, padding: "6px 14px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
            {s === "admin_confirmed" ? "En attente" : s === "injected" ? "Injectés" : "Rejetés"}
          </button>
        ))}
        <button onClick={() => load(statusFilter)} style={{ marginLeft: "auto", background: "var(--sk-surface)", border: "none", borderRadius: 8, padding: "6px 14px", color: "#9ca3af", cursor: "pointer", fontSize: 12 }}>↻</button>
      </div>
      ...
      {imports.map((imp) => {
        ...
        return (
          <div key={imp.id} style={{ background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 12, marginBottom: 10 }}>
            <div style={{ padding: "14px 18px", display: "flex", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 200 }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: "var(--sk-t1)", marginBottom: 3 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
                ...
                <button onClick={() => loadDetail(imp.id)}
                  style={{ background: "var(--sk-surface)", border: "none", borderRadius: 6, padding: "5px 12px", color: "#9ca3af", fontSize: 11, cursor: "pointer" }}>
                ...
                {statusFilter === "admin_confirmed" && (
                  <div style={{ display: "flex", gap: 6, flexDirection: "column", alignItems: "flex-end" }}>
                    <button onClick={() => act(imp.id, "inject")} disabled={acting === imp.id}
                      style={{ background: "#22c55e", color: "var(--sk-deep)", border: "none", borderRadius: 8, padding: "8px 16px", fontWeight: 700, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}>
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      <input placeholder="Motif rejet…" value={rejectReason[imp.id] ?? ""}
                        ...
                        style={{ background: "var(--sk-deep)", border: "0.5px solid var(--sk-surface)", borderRadius: 6, padding: "5px 8px", color: "var(--sk-t1)", fontSize: 11, width: 140 }} />
            ...
            {/* Detail rows */}
            {d && (
              <div style={{ borderTop: "0.5px solid var(--sk-surface)", padding: "12px 18px", overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                  <thead>
                    <tr style={{ background: "var(--sk-deep)" }}>
                  ...
                  <tbody>
                    {(d.parsed_rows ?? []).slice(0, 50).map((r: any) => (
                      <tr key={r.row} style={{ background: r.has_error ? "#1a0808" : r.is_duplicate ? "#1a1400" : "transparent" }}>
                        <td style={{ padding: "5px 10px", color: "var(--sk-t3)" }}>{r.row}</td>
                        <td style={{ padding: "5px 10px", color: "var(--sk-t1)" }}>{r.date}</td>
                        <td style={{ padding: "5px 10px", color: "var(--sk-t1)" }}>{r.driver_name}</td>
...
function PaymentsTab({ tenants, superadminKey, notify }: {
  ...
  const rowStyle: React.CSSProperties = { background: "var(--sk-bg)", border: "0.5px solid var(--sk-surface)", borderRadius: 10, padding: "14px 18px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" };
    ...
    <div>
      ...
      {pending.map(t => (
        <div key={t.id} style={rowStyle}>
          <div style={{ width: 32, height: 32, borderRadius: 6, background: "#f5a623", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "var(--sk-deep)", flexShrink: 0 }}>
            {(t.settings?.app_name || t.name).slice(0, 2).toUpperCase()}
          </div>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <input
              ...
              style={{ background: "#1a1f2e", border: "0.5px solid var(--sk-border)", color: "var(--sk-t1)", borderRadius: 6, padding: "6px 10px", fontSize: 12, width: 60 }}
            />
            <button
              onClick={() => confirmPayment(t.id)}
              disabled={validating === t.id}
              style={{ background: "#22c55e", color: "var(--sk-deep)", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 700, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" }}
            >
